...
**Note:** `my-servers/scripts/deploy-iconoclast-public.sh` rsyncs the main site with `--delete` into the same `www/` tree. That script excludes `gift-for-alif/` so a main-site deploy does not remove this app. If the live URL 404s after an iconoclast deploy, re-run `bash scripts/deploy-from-mac.sh`.
